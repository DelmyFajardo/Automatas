...
class ModernStyle:
    ...
    def apply(root):
        ...
        style.configure("TLabel", background=SECONDARY_COLOR, foreground=TEXT_COLOR, font=("Segoe UI", 10))
        style.configure("TButton", background=BUTTON_COLOR, foreground="white", font=("Segoe UI", 10, "bold"), borderwidth=0, padding=6)
        style.map("TButton",
                  background=[("active", BUTTON_HOVER)],
                  relief=[("pressed", "flat"), ("!pressed", "flat")])
        style.configure("TLabelFrame", background=SECONDARY_COLOR, foreground=PRIMARY_COLOR, font=("Segoe UI", 10, "bold"))
        style.configure("TCombobox", fieldbackground="white", background="white", padding=4, font=("Segoe UI", 10))
        style.configure("Horizontal.TScrollbar", gripcount=0, background=HIGHLIGHT_COLOR)
...
class AutomaApp(tk.Tk):
    ...
    def create_main_menu(self):
        ...
        title = ttk.Label(menu_frame, text="PROYECTO AUTÓMATAS", font=("Segoe UI", 20, "bold"), foreground=PRIMARY_COLOR)
        title.pack(pady=30)
        ttk.Button(menu_frame, text="1. Búsqueda de Patrones (Regex)", command=lambda: self.show_frame(RegexSearchFrame)).pack(pady=10, ipadx=50)
        ttk.Button(menu_frame, text="2. Generación de Palabras (Gramáticas)", command=lambda: self.show_frame(GrammarGenerationFrame)).pack(pady=10, ipadx=50)
        ttk.Button(menu_frame, text="3. Salir", command=self.destroy).pack(pady=20, ipadx=50)
...
class RegexSearchFrame(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        self.source_var = tk.StringVar(value="Ingreso Manual")
        ttk.Label(self, text="BÚSQUEDA DE PATRONES (REGEX)", font=("Segoe UI", 14, "bold"), foreground=PRIMARY_COLOR).pack(pady=10)
        ...
        self.manual_text = scrolledtext.ScrolledText(manual_frame, wrap=tk.WORD, height=8, bg="white", fg=TEXT_COLOR, font=("Consolas", 10))
        self.manual_text.pack(fill="both", expand=True, padx=5, pady=5)
        btn_frame = ttk.Frame(self)
        btn_frame.pack(fill="x", padx=10, pady=10)
        ttk.Button(btn_frame, text="Buscar Patrones", command=self.execute_search).pack(side="left", padx=5)
        ...
        self.output_text = scrolledtext.ScrolledText(output_frame, wrap=tk.WORD, height=10, bg="white", fg=TEXT_COLOR, font=("Consolas", 10))
...
class GrammarGenerationFrame(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        ttk.Label(self, text="GENERACIÓN DE PALABRAS", font=("Segoe UI", 14, "bold"), foreground=PRIMARY_COLOR).pack(pady=10)
        input_frame = ttk.LabelFrame(self, text="Configuración de Generación")
        input_frame.pack(fill="x", padx=10, pady=5)
